...
firebase.auth().onAuthStateChanged(user => {
	if (user) {
		...
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			for (i = 0; i < profitsarray.length; i++) {
				profitsarray[i] = profitsarray[i] || null;
			}
			...
			profits_chart = new Chart(document.getElementById("profits-chart").getContext("2d"), {
			...
			profits_chart.data.labels = dates;
			profits_chart.data.datasets[0].data = profitsarray;

			profits_chart.update();*/
			...
			Chart.controllers.NegativeTransparentLine = Chart.controllers.line.extend({
					update: function () {
						...
						var gradient = ctx.createLinearGradient(0, top, 0, bottom);
						var ratio = Math.min((zero - top) / (bottom - top), 1);
						gradient.addColorStop(0, 'rgba(124,142,49,0.4)');
						gradient.addColorStop(Math.abs(ratio), 'rgba(124,142,49,0.4)');
						gradient.addColorStop(Math.abs(ratio), 'rgba(190,45,45,0.4)');
						gradient.addColorStop(1, 'rgba(190,45,45,0.4)');
						this.chart.data.datasets[0].backgroundColor = gradient;
			...
			for (i = 0; i < profitsarray.length; i++) {
				if (profitsarray[i] !== null) {
					profitsarray[i] = parseFloat(profitsarray[i].toFixed(2));
				}
			}
...
function vars_() {
	return dates + "SPLIT" + profitsarray;
}
